...
function saveOrden($idpedido,$link){
	$queryPedido = mysql_query("SELECT * FROM pedido WHERE idpedido=$idpedido AND estado='pendiente'", $link)or die(mysql_error());
	if (!$queryPedido){
    ...
    while($rows = mysql_fetch_array($queryPedido)){
      ...
      $queryPlatillo = mysql_query("SELECT platillo.nombreplatillo, platillo.precio FROM platillo,orden WHERE orden.idpedido=$idpedido AND platillo.idplatillo = orden.idplatillo", $link)or die(mysql_error());
